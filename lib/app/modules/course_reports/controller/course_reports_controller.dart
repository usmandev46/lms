import 'package:firebase_crashlytics/firebase_crashlytics.dart';
import 'package:flutter/cupertino.dart';
import 'package:get/get.dart';
import '../../../core/constants/app_colors.dart';
import '../../../core/constants/api_endpoints.dart';
import '../../../core/custom_widgets/CustomDataTable.dart';
import '../../../data/repositories/api_service.dart';
import '../model/course_reports.dart';
import '../model/course_reports_filter_model.dart';
import 'package:dio/dio.dart';

class CourseReportsController extends GetxController {
  final page = 1.obs;
  final limit = 17.obs;
  final totalPage = 1.obs;

  var isLoading = false.obs;
  var isFilterLoading = false.obs;

  var courseReportsCourseList = <CourseReportsCourse>[].obs;
  Rx<CourseReportsFilterModel?> courseReportsFilterList = Rx(null);

  var courseReportsColumnList = <String>[].obs;

  Rx<Instructor?> instructor = Rx<Instructor?>(null);
  Rx<CourseFilterCategory?> category = Rx<CourseFilterCategory?>(null);
  Rx<CourseFilterCategory?> subCategory = Rx<CourseFilterCategory?>(null);
  Rx<CourseType?> courseType = Rx<CourseType?>(null);

  final ApiService _api = ApiService();
  final ApiEndpoints _endpoints = ApiEndpoints();


  CancelToken? _reportsToken;
  CancelToken? _filtersToken;

  final Map<String, String> _columnKeyMap = {};
  final Map<int, List<CourseReportsCourse>> _pageCache = {};
  late final CustomTableDataSource<CourseReportsCourse> tableSource;

  @override
  void onInit() {
    super.onInit();
    tableSource = CustomTableDataSource<CourseReportsCourse>(
      columns: courseReportsColumnList,
      jsonMapper: (course) => course.toJson(),
    );
    courseReportsFun();
    courseReportsFiltersFun();
  }

  @override
  void onClose() {
    _reportsToken?.cancel("Reporting screen closed");
    _filtersToken?.cancel("Filters screen closed");
    super.onClose();
  }

  void resetFilters() {
    tableSource.clearRows();

    page.value = 1;
    totalPage.value = 1;

    courseReportsColumnList.clear();
    _columnKeyMap.clear();
    _pageCache.clear();

    instructor.value = null;
    category.value = null;
    subCategory.value = null;
    courseType.value = null;

    courseReportsFun(goToPage: 1);
  }

  void clearCache() {
    _pageCache.clear();
  }

  void courseReportsFun({int? goToPage}) async {
    if (isLoading.value) return;

    final previousPage = page.value;
    if (goToPage != null) {
      page.value = goToPage;
    }

    if (_pageCache.containsKey(page.value)) {
      final cachedStudents = _pageCache[page.value]!;

      tableSource.setPageItems(
        items: cachedStudents,
        columnKeyMap: _columnKeyMap,
        currentPage: page.value,
        pageSize: limit.value,
      );
      return;
    }

    _reportsToken?.cancel();
    _reportsToken = CancelToken();

    isLoading.value = true;

    try {
      var res = await _api.post(_endpoints.courseReports, data: _getFilterMap(),cancelToken: _reportsToken);

      if (res is Map<String, dynamic> && res['success'] == true && res["data"] != null) {

        CourseReportsModel model = CourseReportsModel.fromJson(res);

        totalPage.value = model.data.totalPages ;
        final courses = model.data.courses;

        if (page.value == 1) {
          tableSource.clear();
        }

        if (courseReportsColumnList.isEmpty && courses.isNotEmpty) {
          _generateColumns(courses.first);
        }

        _pageCache[page.value] = courses;

        tableSource.setPageItems(
          items: courses,
          columnKeyMap: _columnKeyMap,
          currentPage: page.value,
          pageSize: limit.value,
        );
      } else {
        page.value = previousPage;
      }

    } catch (e, st) {
      page.value = previousPage;
      if (e is DioException && CancelToken.isCancel(e)) return;
      _handleError("Course Reports", e, st);
    } finally {
      isLoading.value = false;
    }
  }



  Map<String, dynamic> _getFilterMap() {
    final Map<String, dynamic> data = {
      'page': page.value.toString(),
      'limit': limit.value.toString(),
    };
    if (instructor.value != null) data['instructor'] = instructor.value!.registrationNo;
    if (category.value != null) data['category'] = category.value!.id;
    if (subCategory.value != null) data['sub_category'] = subCategory.value!.id;
    if (courseType.value != null) data['type'] = courseType.value!.value;
    return data;
  }

  void courseReportsFiltersFun() async {
    if (isFilterLoading.value) return;

    _filtersToken?.cancel();
    _filtersToken = CancelToken();

    isFilterLoading.value = true;

    try {
      var res = await _api.get(_endpoints.courseFilters, cancelToken: _filtersToken);

      if (res is Map<String, dynamic> && res['success'] == true && res['data'] != null) {
        courseReportsFilterList.value = CourseReportsFilterModel.fromJson(res);
      }
    } catch (e, st) {
      if (e is DioException && CancelToken.isCancel(e)) {
        debugPrint("Filters API cancelled");
        return;
      }
      _handleError("Course Report Filters", e, st);
    } finally {
      isLoading.value = false;
    }
  }

  void _generateColumns(CourseReportsCourse course) {
    final json = course.toJson();

    courseReportsColumnList.assignAll(
        json.keys.map((k) =>
            k.split('_').map((w) => w[0].toUpperCase() + w.substring(1)).join(' '))
    );

    for (int i = 0; i < courseReportsColumnList.length; i++) {
      _columnKeyMap[courseReportsColumnList[i]] = json.keys.elementAt(i);
    }
  }


  void _handleError(String tag, Object e, StackTrace st) {
    FirebaseCrashlytics.instance.recordError(e, st, reason: tag, fatal: false);
    Get.snackbar(
      "Error",
      "Something went wrong",
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: AppColors.error,
      colorText: AppColors.textLight,
    );
  }
}
